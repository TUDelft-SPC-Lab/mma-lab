...
class Indexer(object):
    ...
    def add_audio_to_index(self, vidname, descr):
        ...
        self.con.execute("insert into audiopowers(vidid,features) values (?,?)",
                         (vidid, pickle.dumps(audio)))
    ...
    def add_to_index(self, vidname, descr):
        ...
        # store descriptor per video
        # use pickle to encode NumPy arrays as strings
        self.con.execute("insert into colorhists(vidid,features) values (?,?)",
                         (vidid, pickle.dumps(colhist)))
        self.con.execute("insert into tempdiffs(vidid,features) values (?,?)",
                         (vidid, pickle.dumps(tempdif)))
        self.con.execute("insert into mfccs(vidid,features) values (?,?)",
                         (vidid, pickle.dumps(mfccs)))
        self.con.execute("insert into audiopowers(vidid,features) values (?,?)",
                         (vidid, pickle.dumps(audio)))
        self.con.execute("insert into chdiffs(vidid,features) values (?,?)",
                         (vidid, pickle.dumps(chdiff)))
    ...
    def get_id(self, vidname):
        ...
        if res == None:
            cur = self.con.execute(
                "insert into vidlist(filename) values ('%s')" % basename(vidname))
